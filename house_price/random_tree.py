...
result = [(name, score)for name, score in zip(X_train.columns, rand_clf.feature_importances_)]


sorted(result, key=lambda item: item[1])

for name, score in zip(X_train.columns, rand_clf.feature_importances_):
  print(name, score)
